  ...
  return (
    <div className='bg-wise-purple-dark p-4 overflow-hidden flex flex-col gap-4'>
      ...
      <div className='bg-white p-4 rounded-md shadow-md text-wise-purple-dark flex flex-col gap-4'>
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-4'>
          <div>
            <h2 className='font-bold text-xl mb-4'>Individual Donations.</h2>
            <div className='flex flex-col gap-1 p-4 border border-black/10'>
              {donations.isLoading ? <p>Loading...</p> : null}
              {donations.data?.users.map((user,ix) => (
                <div key={user.id} className={twMerge('flex flex-row items-center gap-4 p-2', ix % 2 === 0 ? 'bg-wise-purple/10' : 'bg-wise-purple/5', ix < 3 ? 'text-xl font-bold' : 'text-lg font-normal')}>
                <div className={twMerge('w-1.5 rounded-full self-stretch -mr-2', ix === 0 ? 'bg-yellow-400' : ix === 1 ? 'bg-slate-400' : ix === 2 ? 'bg-amber-600' : 'bg-wise-purple-dark/10')} />
                <p className='font-black text-wise-purple-dark w-8 text-right'>{ix+1}.</p>
                <div className='flex flex-col items-start'>
                  <p className='flex flex-row items-center gap-2'>{user.firstName} {user.lastName}</p>
                  {user.team ? <p className='text-xs px-2 py-0.5 border border-wise-purple-dark/10 bg-white rounded-md font-normal text-wise-purple-dark/75'>{user.team.name}</p> : null}
                </div>
                <p className='text-2xl ml-auto tracking-wide'>{user.donationPounds ? `£${user.donationPounds}` : '£0'}</p>
              </div>
              ))}
            </div>
          </div>
          <div>
            <h2 className='font-bold text-xl mb-4'>Team Donations.</h2>
            <div className='flex flex-col gap-1 p-4 border border-black/10'>
              {donations.isLoading ? <p>Loading...</p> : null}
              {donations.data?.teams.map((t,ix) => (
              <div key={t.id} className={twMerge('flex flex-row items-center gap-4 p-2', ix % 2 === 0 ? 'bg-wise-purple/10' : 'bg-wise-purple/5', ix < 3 ? 'text-xl font-bold' : 'text-lg font-normal')}>
                <div className={twMerge('w-1.5 rounded-full self-stretch -mr-2', ix === 0 ? 'bg-yellow-400' : ix === 1 ? 'bg-slate-400' : ix === 2 ? 'bg-amber-600' : 'bg-wise-purple-dark/10')} />
                <p className='font-black text-wise-purple-dark w-8 text-right'>{ix+1}.</p>
                <div className="flex flex-col items-start">
                ...
                <p className='text-2xl ml-auto tracking-wide'>£{t.donations}</p>
              </div>
            ))}
            </div>
          </div>
